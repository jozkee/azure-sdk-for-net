...
internal sealed partial class AzureAIInferenceChatClient : IChatClient
{
    ...
    {
        if (chatCompletionsClient is null)
        {
            throw new ArgumentNullException(nameof(chatCompletionsClient));
        }
    ...
    private static ChatCompletionsToolDefinition ToAzureAIChatTool(AIFunction aiFunction)
    {
        ...
        var tool = JsonSerializer.Deserialize(aiFunction.JsonSchema, ChatClientJsonContext.Default.AzureAIChatToolJson)!;
